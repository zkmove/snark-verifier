...
use crate::util::arithmetic::{FieldExt, PrimeField};
    ...
    PoseidonState<F, L, T, RATE>
{
    ...
    fn absorb_with_pre_constants(&mut self, inputs: Vec<L>, pre_constants: &[F; T]) {
        ...
        self.s[0] = L::Loader::sum_with_const(
            self.s[0].loader(),
            &self.s[..1].iter().collect::<Vec<_>>()[..],
            pre_constants[0],
        );
        ...
        {
            *x = L::Loader::sum_with_const(x.loader(), &[x, input], *constant);
        }

        for (i, (x, constant)) in
            self.s.iter_mut().skip(offset).zip(pre_constants.iter().skip(offset)).enumerate()
        {
            *x = L::Loader::sum_with_const(
                x.loader(),
                &[x],
    ...
    fn apply_mds(&mut self, mds: &[[F; T]; T]) {
        let res = mds
            ...
            .map(|row| {
                ...
                L::Loader::sum_with_coeff(
                    a[0].1.loader(),
                    &a.iter().map(|(c, b)| (*c, b)).collect::<Vec<_>>()[..],
                )
            })
            .collect::<Vec<_>>();
    ...
    fn apply_sparse_mds(&mut self, mds: &SparseMDSMatrix<F, T, RATE>) {
        ...
        let mut res = vec![L::Loader::sum_with_coeff(
            a[0].1.loader(),
            &a.iter().map(|(c, b)| (*c, b)).collect::<Vec<_>>()[..],
        )];

        for (e, x) in mds.col_hat().iter().zip(self.s.iter().skip(1)) {
            res.push(L::Loader::sum_with_coeff(x.loader(), &[(*e, &self.s[0]), (F::one(), x)]));
        }
